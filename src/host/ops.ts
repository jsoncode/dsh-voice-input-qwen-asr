...
import { STEP_IDS, type Installer, type StepId } from './installer.ts'
...
import { saveStore, type VoiceStore } from './store.ts'
import { inspectInstalled, inspectUsable } from './paths.ts'
...
export function sanitizeOverrides(raw: unknown): VoiceConfigOverrides {
...
export async function runOp(deps: OpsDeps, op: string, params: Record<string, unknown>): Promise<unknown> {
  switch (op) {
    case 'config.get':
    ...
    case 'install.start': {
      const steps = Array.isArray(params.steps) ? (params.steps as StepId[]) : []
