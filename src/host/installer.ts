...
export const STEP_IDS: readonly StepId[] = ['runtime', 'model', 'venv', 'deps']
...
export class Installer {
  ...
  private queue: StepId[] = []
  private currentStep: StepId | null = null
  private readonly states = new Map<StepId, StepState>(STEP_IDS.map((id) => [id, 'idle' as StepState]))
  private readonly exitCodes = new Map<StepId, number | null>(STEP_IDS.map((id) => [id, null]))
  ...
  status(): InstallerStatus {
    const steps = Object.fromEntries(
      STEP_IDS.map((id) => [id, { state: this.states.get(id) ?? 'idle', exitCode: this.exitCodes.get(id) ?? null }]),
  ...
  start(steps: StepId[]): { started: boolean; error?: string } {
    if (this.running) return { started: false, error: '已有安装任务在进行中' }
    const wanted = steps.filter((step) => STEP_IDS.includes(step))
    if (wanted.length === 0) return { started: false, error: '没有可执行的安装步骤' }
    for (const step of wanted) {
      if (this.states.get(step) !== 'done') this.states.set(step, 'idle')
    }
    this.queue = [...wanted]
    this.running = true
    void this.drain()
  ...
  private async runStep(step: StepId): Promise<void> {
  ...
  private stepRuntime(paths: VoicePaths): Promise<void> {
  ...
  private stepModel(paths: VoicePaths): Promise<void> {
  ...
  private async stepVenv(paths: VoicePaths): Promise<void> {
  ...
  private getPipIndexArgs(): string[] {
   ...
   */
  private exec(argv: string[], cwd: string): Promise<void> {
    return new Promise<void>((resolveP, rejectP) => {
      ...
      child.stdout?.on('data', (chunk: string) => this.pushLog(chunk))
