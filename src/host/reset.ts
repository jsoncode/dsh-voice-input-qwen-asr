...
export class EnvReset {
  ...
  async run(getConfig: () => VoicePluginConfig, getPaths: () => VoicePaths, asr: AsrServer, installer: Installer): Promise<ResetResult> {
    if (this.resetting) return { ok: false, error: '环境重置正在进行中' }
    if (installer.status().running) return { ok: false, error: '安装任务进行中，请等待完成或稍后再试' }
    this.resetting = true
    try {
      asr.stop()
