...
function normalize(value: unknown): Lang {
...
export function initI18n(ctx: {
  ...
  try {
    const locale = ctx.get('locale') as { getSnapshot?: () => { active?: unknown } } | undefined
    const snapshot = locale?.getSnapshot?.()
    if (snapshot && snapshot.active !== undefined) setLang(normalize(snapshot.active))
  } catch { /* keep default */ }
  if (typeof document !== 'undefined' && lang === 'zh') {
    setLang(normalize(document.documentElement.lang || 'zh'))
  }
  ctx.on?.('locale/change', (...args: unknown[]) => {
    const next = args[0]
    setLang(normalize(typeof next === 'string' ? next : (next as { active?: unknown } | undefined)?.active))
  })
}
...
export function onLangChange(notify: () => void): () => void {
