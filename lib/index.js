...
/** 依赖安装完成标记（当前名）。 */
const DEPS_MARKER_NAME = ".dsh-voice-input-qwen-asr-deps-ok";
/** 依赖标记旧名（v0.1.0 包名 dsh-voice-input 时代写入），升级用户的环境据此仍判已安装。 */
const LEGACY_DEPS_MARKER_NAME = ".dsh-voice-input-deps-ok";
function resolvePaths(config, storeDir, asrScript) {
	...
	return {
		...
		depsMarker: join(runtimeDir, DEPS_MARKER_NAME)
	};
}
/** 依赖是否已安装：当前名或旧名标记任一存在（旧名来自 v0.1.0 升级环境）。 */
function depsInstalled(paths) {
	return existsSync(paths.depsMarker) || existsSync(join(paths.runtimeDir, ".dsh-voice-input-deps-ok"));
}
/** 以文件系统实测安装状态（不做双份记账）。 */
...
* 本文件不含任何绝对路径。
*/
const name = "dsh-voice-input-qwen-asr";
const inject = [
	"webServer",
	"connection",
	"settings"
