...
import { isLfsPointer } from "./paths.js";
...
export class AsrServer {
    ...
    async start() {
        ...
        if (!existsSync(paths.venvPython))
            throw new Error('虚拟环境未创建，请先在设置页完成安装步骤');
        if (!existsSync(paths.modelDir))
            throw new Error('模型库未克隆，请先在设置页完成安装步骤');
        if (!existsSync(this.asrScript))
            throw new Error('插件损坏：缺少 python/asr_server.py');
    ...
    checkModelCloned(modelDir) {
        try {
            ...
            if (e instanceof Error && e.message.includes('git-lfs'))
                throw e;
