...
export async function addVoiceRecord(record) {
...
export async function deleteVoiceRecords(ids) {
    const db = await openDb();
    try {
        const tx = db.transaction(STORE, 'readwrite');
...
export async function clearVoiceRecords() {
    const db = await openDb();
    try {
        const tx = db.transaction(STORE, 'readwrite');
 ...
 */
export function buildWavBlob(chunks, sampleRate = 16000) {
    let totalBytes = 0;
